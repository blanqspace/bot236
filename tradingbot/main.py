...
from tradingbot.commands.telegram import TelegramCommand
...
def handle_command(text: str, state: BotState, orders: OrderManager):
    ...
    if cmd == "/status":
        ...
        symbol = parts[1].upper()
        qty = int(parts[2])
        orders.submit(symbol, "SELL", qty)
    elif cmd == "/pause" and len(parts) >= 2:
        symbol = parts[1].upper()
        state.ensure_symbol(symbol).paused = True
        logger.info("Paused %s", symbol)
    elif cmd in ("/resume", "/unpause") and len(parts) >= 2:
        symbol = parts[1].upper()
        state.ensure_symbol(symbol).paused = False
...
def main():
    ...
    terminal.start()
    telegram = TelegramCommand(lambda txt: handle_command(txt, state, orders))
    telegram.start()
